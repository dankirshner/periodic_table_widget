...
(function($) {
    ...
    $.fn.ZoomArea = function(options) {
        ...
        function touchDownCalls(event) {
            if (debug) {
               console.log ('[touchDownCalls] event:', event);
               console.log ('[touchDownCalls] event.originalEvent.touches.length:', event.originalEvent.touches.length);
            }
            enableDragging();
        ...
        function touchMoveCalls(event) {
            if (debug) {
               console.log ('[touchMoveCalls] event:', event);
            }
            logTouchPosition(event);
        ...
        function logTouchPosition(event) {
            ...
            currentMousePos.x = event.touches[0].pageX;
            currentMousePos.y = event.touches[0].pageY;
            if (debug) {
               console.log ('[logTouchPosition] event.touches[0]:', event.touches[0]);
            }
        }

        function dragWithFinger(event) {
            if (debug) {
               console.log ('[dragWithFinger] event:', event);
            }
        ...
        function processPinch(event) {
            ...
            if (typeof event.touches[1] == 'undefined') {
                return false;
            }
        ...
        function dragWithMouse(event) {
            ...
            if (debug) {
               console.log ('[dragWithMouse] event:', event);
            }
            settings.dragged = true;
        ...
        function logMousePosition(event) {
            ...
            //console.log ('[logMousePosition] currentMousePos.x:', currentMousePos.x);
        ...
        function zoomingToFit(_element) {
            ...
            if (debug) {
               console.log ('[zoomingToFit] settings.zoomLevel:', settings.zoomLevel);
            }
        ...
        function zoomingToPoint(_element, zoomLevel, coordinateXPercent, coordinateYPercent) {
            ...
            var coordinateY = settings.height * (coordinateYPercent/100);
    	 ...
    	 * to center INITIALLY
    	*****************************************************/
        function init(_element, real_init_f) {
	    ...
	    var elementParentOffsets;
            ...
            //}
            if (real_init_f) {
               settings.initialZoomLevel = settings.zoomLevel;
               settings.minZoomLevel     = settings.zoomLevel;
            }
    	...
    	*****************************************************/
        ...
        return {
            ...
            zoomToPoint:   function(zoomLevel, centerX, centerY) {
